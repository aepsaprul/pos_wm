            ...
            @if (Auth::user()->employee_id != null)
                <div class="card-header">
                  <div class="d-flex justify-content-between">
                    ...
                    <div>
                      <input type="date" name="start_date" id="start_date" class="rounded" style="border: 1px solid lightgray;">
                      s/d
                      <input type="date" name="end_date" id="end_date" class="rounded" style="border: 1px solid lightgray;">
                      <button id="btn_cari" class="rounded bg-primary px-3 ml-2" style="border: 1px solid lightblue;"><i class="fas fa-search"></i></button>
                    </div>
                  </div>
                </div>
            @endif
...
<script>
  $(document).ready(function() {
    ...
    const btn_cari = document.querySelector('#btn_cari');
    const wrapper_table = document.querySelector('#wrapper_table');
    const cardBody = document.querySelector('.card-body');

    btn_cari.addEventListener('click', function (e) {
      e.preventDefault();
      wrapper_table.classList.add('d-none');
      cardBody.innerHTML = '';
      ...
      $.ajax({
        url: "{{ URL::route('inventory_invoice.filterTanggal') }}",
        type: "post",
        data: formData,
        success: function(response) {
          ...
          let val = `
            <table id="datatable" class="table table-striped table-bordered" style="width:100%; font-size: 14px;">
              ...
              <tbody>`;
              inventories.forEach(function (inv, i) {
                val += `
                  <tr>
                    <td class="text-center">${i + 1}</td>
                    <td class="text-center">${new Intl.DateTimeFormat('id-ID').format(new Date(inv.date_recorded))}</td>
                    <td>${inv.shop.name}</td>
                    <td>${inv.user ? inv.user.name : ''}</td>
                    <td class="text-center"><a href="#" class="btn-detail" data-id="${inv.id}">${inv.code}</a></td>
                    <td class="text-right">${new Intl.NumberFormat('id-ID').format(inv.total_amount)}</td>
                    <td class="text-center">${inv.product_out.length > 0 ? inv.product_out[0].qty : ''}</td>
                    <td class="text-center text-uppercase">${inv.payment_methods}</td>
                    <td class="text-center">`;
                      if (inv.status === "unpaid") {
                        val += `
                          <button type="button" id="btn-cancel-${inv.id}" class="btn text-capitalize rounded bg-gradient-warning px-3 btn-cancel" data-id="${inv.id}" style="width: 120px;">cancel</button>
                          <button class="btn btn-default btn-unpaid-spinner-${inv.id} d-none" disabled style="width: 120px;">
                            <span class="spinner-grow spinner-grow-sm"></span>
                              Loading..
                          </button>
                    ...
                    <td class="text-center">
                      <a href="#" class="text-uppercase" id="status_transaksi" data-id="${inv.id}">${inv.status_transaksi ? inv.status_transaksi.nama : ''}</a>
                    </td>
                    <td class="text-center">
                      <div class="btn-group">
                        <a
                        ...
                        const id = inv.id;
                        let url = '{{ route("inventory_invoice.print", ":id") }}';
          ...
          `;
          cardBody.insertAdjacentHTML('afterbegin', val);
        }
      })
    })
